...
const register = async(req,res) => {
  try{
   ...
   const user = await User.create(req.body);
     //send tokens
  const token = jwt.sign({_id: user._id , emailId: user.emailId, role: user.role},process.env.JWT_SECRET, {expiresIn: "1h"});
...
const adminRegister = async (req, res) => {
  try{
    ...
    const user = await User.create (req.body);
    const token = jwt.sign({_id: user.id , emailId: user.emailId, role: user.role }, process.env.JWT_SECRET, {expiresIn: "1h"});
